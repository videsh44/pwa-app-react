...
export default class User_location extends Component {
  ...
  async componentDidMount() {
    ...
    inputPlaceHolders();
    if (
      (this.props.location.state && this.props.location.state.credential) ||
      (typeof getId_user !== 'undefined' && getId_user) ||
      (this.props.location.state &&
        this.props.location.state.personal.user_data.ID)
    ) {
      try {
        let getuseraddress = await axios.get('/api/ca/v1/users', {
          params: {
            user_id:
              getId_user || this.props.location.state.personal.user_data.ID,
          },
        });
        ...
        this.setState({
          ...
          street_address:
            getuseraddress.data.data.user_other_info.street_address[0],
          ...
          address_line_2:
            getuseraddress.data.data.user_other_info.address_line2[0],
        });
      } catch (error) {}
    } else if (
      this.props.location.state &&
  ...
  validateRegisterParam = () => {
    ...
    if (isStreetaddress(this.state))
      errors.street = 'Please enter Your Street Address';
    if (validatecountry(this.state))
    ...
    else if (!Zipcodevalidate(this.state))
      errors.zip = 'Please enter Correct Your Zip Code';
  ...
  redirectmethod = async () => {
    ...
    if (getroles === 'User') {
      this.props.history.push(
        `/home_page/${
          this.props.location.state.credential.FirstName +
          '%20' +
          this.props.location.state.credential.LastName
        }`
      );
    }
  };
  ...
  render() {
    ...
    if (
      (this.state.changeoption &&
        this.props.location.state &&
        this.props.location.state.credential) ||
      (this.state.changeoption &&
        this.props.location.state &&
        this.props.location.state.pass &&
        this.props.location.state.personal) ||
                ...
                <div className="form-block formBox">
                  <fieldset>
                    ...
                    <input
                      ...
                      autoComplete="off"
                    />
                    <label className="error" style={{ display: 'block' }}>
                      {' '}
